...
def dbx_api(token, endpoint, payload):
    ...
    if r.status_code >= 400:
        raise RuntimeError(f"Dropbox API error en {endpoint}: {r.text}")
...
def dbx_ensure_folder(token, path):
    try:
        dbx_api(token, "files/create_folder_v2", {"path": path, "autorename": False})
...
def ensure_base_folders(token):
    for folder in [INPUT_FOLDER, PROCESSED_FOLDER, ERROR_FOLDER, OUTPUT_FOLDER]:
        dbx_ensure_folder(token, folder)
...
def extract_with_regex(text):
    ...
    for pat in [
        r"(?:n[uú]mero de factura|num\.?\s*factura|factura|invoice|n[ºo°])[\s:.-]*([A-Z0-9\-/]+)",
    ]:
    ...
    patterns = {
        ...
        "total": [
            r"(?:importe total|total factura|total)[^\d]{0,15}([\d\.\,]+)",
        ],
    }
    ...
    for line in text.splitlines():
        ...
        if re.search(r"\d{2}[/-]\d{2}[/-]\d{4}", line):
            continue
        if re.search(r"(factura|invoice|base imponible|iva|total)", line, re.IGNORECASE):
            continue
...
def merge_data(primary, fallback, dropbox_path):
    ...
    return {
        ...
        "proveedor": pick("supplier", "issuer", "seller", "from"),
...
def has_useful_data(row):
    for k, v in row.items():
        if k != "archivo_dropbox" and str(v).strip():
            return True
    return False
...
def process_file(token, entry):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        original_text = pdf_to_text(pdf_input)

        if original_text:
            inv = try_invoice2data(pdf_input)
            reg = extract_with_regex(original_text)
            merged = merge_data(inv, reg, path_display)
            if has_useful_data(merged):
                return merged
            raise ValueError("PDF con texto pero sin datos útiles extraíbles")

        ocr_pdf = td / f"{pdf_input.stem}_ocr.pdf"
        ocr_result = run_ocr(pdf_input, ocr_pdf)

        if ocr_result.returncode != 0:
            stderr = ((ocr_result.stderr or "") + "\n" + (ocr_result.stdout or "")).strip()

            if "DigitalSignatureError" in stderr or "digital signature" in stderr.lower():
                inv = try_invoice2data(pdf_input)
                reg = extract_with_regex(original_text)
                merged = merge_data(inv, reg, path_display)
                if has_useful_data(merged):
                    return merged
                raise ValueError("PDF firmado digitalmente y sin texto util extraíble")

            raise ValueError(f"Fallo OCR: {stderr}")

        ocr_text = pdf_to_text(ocr_pdf)
        inv = try_invoice2data(ocr_pdf)
        reg = extract_with_regex(ocr_text)
        merged = merge_data(inv, reg, path_display)

        if has_useful_data(merged):
            return merged

        raise ValueError("OCR completado pero sin datos útiles extraíbles")


def main():
    token = get_access_token()
    ensure_base_folders(token)
    ...
    existing_rows = read_existing_rows(dbx_download_if_exists(token, OUTPUT_CSV))
    added_rows = 0
    ...
    if existing_rows:
        dbx_upload(token, write_csv(existing_rows), OUTPUT_CSV)
        print(f"CSV actualizado correctamente. Filas nuevas: {added_rows}")
    else:
        print("No se ha añadido ninguna fila al CSV")
